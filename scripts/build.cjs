...
const pricing=services.slice(1).map((s,i)=>`<details class="tariff" name="site-tariff"><summary><span class="row-number">${String(i+1).padStart(2,'0')}</span><span class="tariff-title">${escape(s.name)}</span><span class="tariff-short">${escape(s.short)}</span><span class="tariff-icon" aria-hidden="true"></span></summary><div class="tariff-body"><p class="tariff-price">${escape(s.price)}</p><dl class="tariff-facts"><div><dt>Для каких задач</dt><dd>${escape(s.suitable)}</dd></div><div><dt>Что входит</dt><dd>${escape(s.includes)}</dd></div><div><dt>Можно добавить</dt><dd>${escape(s.extras)}</dd></div></dl><a href="#contact" class="button button-outline">Обсудить проект <span aria-hidden="true">↗</span></a></div></details>`).join('\n');
